...
@Slf4j
@Service(value = "zIncrByHandler")
public class ZIncrByHandler extends BaseHandler {
    ...
    public String handle(List<String> commandList) {
        ...
        double delta = Double.parseDouble(commandList.get(1));
        String member = commandList.get(2);

        return redisService.getSortedSetRedisService().zincrBy(key, delta, member);
    }
