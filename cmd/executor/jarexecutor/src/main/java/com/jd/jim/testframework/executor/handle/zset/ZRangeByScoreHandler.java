...
@Slf4j
@Service(value = "zRangeByScoreHandler")
public class ZRangeByScoreHandler extends BaseHandler {
    ...
    public String handle(List<String> commandList) {
        ...
        if (StringUtils.isNumeric(minStr) && StringUtils.isNumeric(maxStr)) {
            ...
            return redisService.getSortedSetRedisService().zrangeByScore(key, min, max);
        } else {
            return redisService.getSortedSetRedisService().zrangeByScore(key, minStr, maxStr);
        }
    }
