...
@Slf4j
@Service("hIncrByHandler")
public class HIncrByHandler extends BaseHandler {
    @Override
    public String handle(List<String> commandList) {
        ...
        long delta = Long.valueOf(commandList.get(2));

        return redisService.getHashRedisService().hincrBy(key, field, delta);
    }
}
