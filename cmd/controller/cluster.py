...
class cluster:
    ...
    def __assign(self):
        ...
        for opt in self.opts[1:]:
            ...
            opt.executor_count = self.opt.executor_count // n
            opt.differ_count = self.opt.differ_count // n
            self.opts[0].generator_count -= opt.generator_count
            self.opts[0].executor_count -= opt.executor_count
            self.opts[0].differ_count -= opt.differ_count

        for i in range(n):
            print("ASSIGN COUNT:::", i, self.opts[i].generator_count, self.opts[i].executor_count, self.opts[i].differ_count)
